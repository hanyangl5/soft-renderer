...
public:
	...
	glm::vec4 GetPos(uint32_t ind) {
		return triangle[ind].position;
	}
	glm::vec4 GetNormal(uint32_t ind) {
		return triangle[ind].normal;
	}
	glm::vec2 GetTexcoord(uint32_t ind) {
		return triangle[ind].tex_coord;
	}
private:
