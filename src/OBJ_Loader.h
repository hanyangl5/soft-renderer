...
inline bool SameSide(Vector3 p1, Vector3 p2, Vector3 a, Vector3 b) {
    Vector3 cp1 = srmath::CrossV3(b - a, p1 - a);
    Vector3 cp2 = srmath::CrossV3(b - a, p2 - a);

    return srmath::DotV3(cp1, cp2) >= 0;
}
...
inline Vector3 GenTriNormal(Vector3 t1, Vector3 t2, Vector3 t3) {
    ...
    Vector3 normal = srmath::CrossV3(u, v);
...
inline bool inTriangle(Vector3 point, Vector3 tri1, Vector3 tri2, Vector3 tri3) {
    ...
    Vector3 proj = srmath::ProjV3(point, n);

    // If the distance from the triangle to the point is 0
    //	it lies on the triangle
    if (srmath::MagnitudeV3(proj) == 0)
        return true;
    else
        return false;
}
  ...
  private:
    ...
    void GenVerticesFromRawOBJ(std::vector<Vertex> &oVerts, const std::vector<Vector3> &iPositions,
        ...
        if (noNormal) {
            ...
            Vector3 normal = srmath::CrossV3(A, B);
    ...
    void VertexTriangluation(std::vector<unsigned int> &oIndices, const std::vector<Vertex> &iVerts) {
        ...
        while (true) {
            ...
            for (int i = 0; i < int(tVerts.size()); i++) {
                ...
                float angle = srmath::AngleBetweenV3(pPrev.Position - pCur.Position, pNext.Position - pCur.Position) *
                              (180 / 3.14159265359);
